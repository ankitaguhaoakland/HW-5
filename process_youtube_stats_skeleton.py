...
def process_youtube(fn_youtube , fn_out, category):
    """
    Function
    --------
    process_youtube
    ...
    re_filter = re.compile(r',([0-9]{1,2}\.[0-9]{1,2}\.[0-9]{1,2}),"(\w*\s*)*","(\w*\s*)*",28',re.I)
    ...
    with open(fn_youtube) as yt:
        ...
        for line in yt:
            ...
            if mfilter:
                ...
                outputlines = outputlines.append(line)
    ...
    fn_out.writelines("%s\n" % item for item in outputlines)

    # # Print out the counts dictionary
    for lk in daily_counts:
        print (lk, ":", lk[])
